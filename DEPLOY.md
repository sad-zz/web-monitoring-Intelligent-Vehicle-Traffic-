...
## پیکربندی IP این استقرار

برای این استقرار، تنظیم هدف به این شکل است:

- **دریافت داده از دستگاه‌ها:** `5.159.49.246:2022`
- **ارسال داده به RMTO:** با **Source IP** برابر `5.159.49.71`
...
اگر پنل برنامه بالا نمی‌آید، تنظیم‌ها را مستقیم در دیتابیس اعمال کنید:

```bash
cd /opt/tc-manager/server
sqlite3 data.db "
UPDATE settings SET value='5.159.49.246' WHERE key='server_ip';
UPDATE settings SET value='2022' WHERE key='tcp_port';
UPDATE settings SET value='5.159.49.71' WHERE key='rmto_source_ip';
"
systemctl restart tc-manager
```

> **مهم:** IP `5.159.49.71` باید واقعا روی همان سرور قابل استفاده باشد. اگر در خروجی `ip a` دیده نمی‌شود، آن را به‌عنوان **secondary IP** در تنظیمات شبکه/پنل VPS اضافه کنید؛ صرف ذخیره‌کردن آن داخل برنامه کافی نیست.
