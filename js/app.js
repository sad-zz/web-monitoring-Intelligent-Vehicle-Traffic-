...
// ============================================================
    ...
    function initTestSender() {
        ...
        function loadProxyDevices() {
            ...
            // Load online devices (+ all devices for source selection since an offline device may have recent data too)
            api("GET", "/api/devices", null, function (status, data) {
                var sel = $("#proxy-source-device");
                if (!sel || !data) return;
                proxyOnlineDevicesCache = data;
                sel.innerHTML = '<option value="">-- انتخاب دستگاه منبع --</option>';
                // Show online devices first, then others
        ...
        // Show device route info when offline device is selected
