...
// ============================================================
// Fix39: SOAP datetime normalization + auth-error fast-fail + skip rid=0
// ============================================================

// Fix39a: add toSoapDateTime helper + use it for ST/ET in sendAdd5
patch("Fix39a: toSoapDateTime() + use it for ST/ET in sendAdd5 (fixes XML AllXsd error)",
    "server/rmto-client.js",
    'function sendAdd5(data, callback) {\n    ensureClient(function (err) {\n        if (err) return callback(err);\n\n        var isNull = (data.c1 === null);\n        var args = {\n            CID: parseInt(COMPANY_CODE, 10) || 58,\n            UID: USERNAME,\n            PWD: PASSWORD,\n            FID: data.fid,\n            RID: data.rid,\n            ST: data.st,\n            ET: data.et,',
...
// Fix39b: early exit if no credentials + skip rid=0 records in scheduler query
patch("Fix39b: early-exit if no RMTO credentials + skip rid=0 records in send query",
    "server/scheduler.js",
...
// Fix39c: fix SRVDT storage (was String(Date) = garbled) + catch "http status codes" as auth error
patch("Fix39c: normalize SRVDT storage + catch http-status-codes as auth error",
    "server/scheduler.js",
